...
foreach ( $routes_by_page as $page_slug => $page_routes ) {
	...
	$global_name = 'ai_' . $page_slug_underscore . '_routes_data';
 ...
 */
function ai_register_page_routes( $page_routes, $register_function_name ) {
	...
	foreach ( $page_routes as $route ) {
		...
		if ( $route['has_content'] ) {
			...
			if ( file_exists( $content_asset_path ) ) {
				...
				$content_handle = 'ai/routes/' . $route['name'] . '/content';
		...
		if ( $route['has_route'] ) {
			...
			if ( file_exists( $route_asset_path ) ) {
				...
				$route_handle = 'ai/routes/' . $route['name'] . '/route';
				$extension = defined( 'SCRIPT_DEBUG' ) && SCRIPT_DEBUG ? '.js' : '.min.js';
				// Deregister first to override any previously registered version
				// (e.g., Core's default modules when running as a plugin).
...
// Page-specific route registration functions for ai
/**
 * Register routes for ai page (full-page mode).
 */
function ai_register_ai_page_routes() {
	global $ai_ai_routes_data;
	ai_register_page_routes( $ai_ai_routes_data, 'ai_register_ai_route' );
}
add_action( 'ai_init', 'ai_register_ai_page_routes' );

/**
 * Register routes for ai page (wp-admin mode).
 */
function ai_register_ai_wp_admin_page_routes() {
	global $ai_ai_routes_data;
	ai_register_page_routes( $ai_ai_routes_data, 'ai_register_ai_wp_admin_route' );
}
add_action( 'ai-wp-admin_init', 'ai_register_ai_wp_admin_page_routes' );
